...
void Buffers::handlePinVolChanges(MicroCom::Pins pin, Voltage value){
    ...
    if(bftype == MicroCom::BF_74LS244){
        if(pin==MicroCom::BF4_Ga || pin==MicroCom::BF4_Gb){
            ...
            //结果
            qDebug()<<"1A1="<<pins[MicroCom::BF4_1O1];
            qDebug()<<"1A2="<<pins[MicroCom::BF4_1O2];
            qDebug()<<"1A3="<<pins[MicroCom::BF4_1O3];
            qDebug()<<"1A4="<<pins[MicroCom::BF4_1O4];
            qDebug()<<"2A1="<<pins[MicroCom::BF4_2O1];
            qDebug()<<"2A2="<<pins[MicroCom::BF4_2O2];
            qDebug()<<"2A3="<<pins[MicroCom::BF4_2O3];
            qDebug()<<"2A4="<<pins[MicroCom::BF4_2O4];
        }
        else{
            return;
        }
        ...
        if(pin==MicroCom::BF5_g){
            ...
            if(value==low){
                ...
                if(Buffers::getPinVoltage(MicroCom::BF5_DIR)==high){
                    ...
                    qDebug()<<"B0="<<pins[MicroCom::BF5_B0];
                    qDebug()<<"B1="<<pins[MicroCom::BF5_B1];
                    qDebug()<<"B2="<<pins[MicroCom::BF5_B2];
                    qDebug()<<"B3="<<pins[MicroCom::BF5_B3];
                    qDebug()<<"B4="<<pins[MicroCom::BF5_B4];
                    qDebug()<<"B5="<<pins[MicroCom::BF5_B5];
                    qDebug()<<"B6="<<pins[MicroCom::BF5_B6];
                    qDebug()<<"B7="<<pins[MicroCom::BF5_B7];
                }
                //如果DIR为低电平，则数据从B(108~115,pins[8~15])端口->A(100~107,pins[0~7])端口
                    ...
                    qDebug()<<"A0="<<pins[MicroCom::BF5_A0];
                    qDebug()<<"A1="<<pins[MicroCom::BF5_A1];
                    qDebug()<<"A2="<<pins[MicroCom::BF5_A2];
                    qDebug()<<"A3="<<pins[MicroCom::BF5_A3];
                    qDebug()<<"A4="<<pins[MicroCom::BF5_A4];
                    qDebug()<<"A5="<<pins[MicroCom::BF5_A5];
                    qDebug()<<"A6="<<pins[MicroCom::BF5_A6];
                    qDebug()<<"A7="<<pins[MicroCom::BF5_A7];
                }
            }
            //如果不是低电平（非有效）
            else{
